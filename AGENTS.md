...
6. **DO NOT Hand-Transcribe Digest Values, File Names, or Test Counts in Reports**: Never retype digest hex strings, algorithm file names (e.g. referencing display name `triple-des.test.ts` instead of actual filename `3des.test.ts`), or per-file test counts from memory, internal state, or asynchronous streaming console progress lines (which frequently interleave chunks from different suites like `sha3-256` vs `sha256`). Always copy directly from unedited final test runner outputs or deterministic per-file runs (`npx vitest run <file>`) and exact filesystem listings (`ls tests/algorithms/`). The raw output is the ground truth; the table is supplemental readability only.
