...
describe('NIST FIPS 203 ML-KEM (CRYSTALS-Kyber)', () => {
  const seedD = hexToBytes(NIST_FIPS203_KAT_SEED_D);
  const seedZ = hexToBytes(NIST_FIPS203_KAT_SEED_Z);
  const seedM = hexToBytes(NIST_FIPS203_KAT_SEED_M);
  ...
  describe('ML-KEM-512 (NIST Category 1)', () => {
    it('executes deterministic KeyGen, Encapsulation, and Decapsulation round-trip', () => {
      ...
      expect(bytesToHex(recoveredKey)).toBe(bytesToHex(sharedKey));
      expect(decapsSteps.length).toBeGreaterThan(0);
    });

    it('computes expected digests via plugin API', () => {
      const keyGenRes = mlKem512KeyGenPlugin.compute('');
      const encapsRes = mlKem512EncapsulatePlugin.compute('');
      const decapsRes = mlKem512DecapsulatePlugin.compute('');

      expect(keyGenRes.digest.length).toBe(1600); // 800 bytes hex
      expect(encapsRes.digest.length).toBe(64);   // 32 bytes hex
      expect(decapsRes.digest).toBe(encapsRes.digest);
  ...
  describe('ML-KEM-768 (NIST Category 3 / Recommended)', () => {
    ...
    it('computes expected digests via plugin API', () => {
      const keyGenRes = mlKem768KeyGenPlugin.compute('');
      const encapsRes = mlKem768EncapsulatePlugin.compute('');
      const decapsRes = mlKem768DecapsulatePlugin.compute('');
  ...
  describe('ML-KEM-1024 (NIST Category 5)', () => {
    ...
    it('computes expected digests via plugin API', () => {
      const keyGenRes = mlKem1024KeyGenPlugin.compute('');
      const encapsRes = mlKem1024EncapsulatePlugin.compute('');
      const decapsRes = mlKem1024DecapsulatePlugin.compute('');
